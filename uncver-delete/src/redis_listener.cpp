                ...
                if (messages->type == REDIS_REPLY_ARRAY) {
                    for (size_t j = 0; j < messages->elements; ++j) {
                        ...
                        if (msg->type == REDIS_REPLY_ARRAY && msg->elements >= 2) {
                            DeleteRequest request = parseRequest(msg->element[1]);
                            
                            if (callback_) {
                                DeleteResponse response = callback_(request);
...
DeleteRequest RedisListener::parseRequest(redisReply* reply) {
    ...
    for (size_t i = 0; i < reply->elements - 1; i += 2) {
        ...
        if (key->type == REDIS_REPLY_STRING && 
            ...
            try {
                ...
                if (j.contains("artifact_name")) {
                    request.artifact_name = j["artifact_name"].get<std::string>();
                }
...
bool RedisListener::publishResponse(const DeleteResponse& response) {
    json j;
    j["request_id"] = response.request_id;
    j["artifact_name"] = response.artifact_name;
...
void RedisListener::sendError(const DeleteResponse& error) {
    json j;
    j["request_id"] = error.request_id;
    j["artifact_name"] = error.artifact_name;
    j["success"] = false;
...
void RedisListener::onDeleteRequest(std::function<DeleteResponse(const DeleteRequest&)> callback) {
    callback_ = callback;
}
