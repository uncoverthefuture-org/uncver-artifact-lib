...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Uncver Delete Artifact Service" << std::endl;
    ...
    // Create artifact deleter
    auto deleter = std::make_unique<ArtifactDeleter>();

    // Set up callback for delete requests
    listener->onDeleteRequest([&deleter](const DeleteRequest& request) -> DeleteResponse {
        return deleter->deleteArtifact(request);
    });

    std::cout << "Listening for delete requests..." << std::endl;
