                ...
                if (messages->type == REDIS_REPLY_ARRAY) {
                    for (size_t j = 0; j < messages->elements; ++j) {
                        ...
                        if (msg->type == REDIS_REPLY_ARRAY && msg->elements >= 2) {
                            StartRequest request = parseRequest(msg->element[1]);
                            
                            if (callback_) {
                                StartResponse response = callback_(request);
...
StartRequest RedisListener::parseRequest(redisReply* reply) {
...
bool RedisListener::publishResponse(const StartResponse& response) {
    json j;
...
void RedisListener::sendError(const StartResponse& error) {
    json j;
...
void RedisListener::onStartRequest(std::function<StartResponse(const StartRequest&)> callback) {
    callback_ = callback;
}
