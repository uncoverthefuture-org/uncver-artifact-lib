...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Uncver Stop Artifact Service" << std::endl;
    ...
    // Create artifact stopper
    auto stopper = std::make_unique<ArtifactStopper>();

    // Set up callback for stop requests
    listener->onStopRequest([&stopper](const StopRequest& request) -> StopResponse {
        return stopper->stopContainer(request);
    });

    std::cout << "Listening for stop requests..." << std::endl;
